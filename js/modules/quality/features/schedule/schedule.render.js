...
(function () {
  ...
  window.rbi_renderScheduleTab = async function (skipLoad = false) {
    ...
    let rowsHtml = activeData.sort((a, b) => new Date(a.startDate || 0) - new Date(b.startDate || 0)).map(s => {
        ...
        return `
            <tr class="sched-row hover:bg-[var(--hover-bg)] transition-colors" data-id="${s.id}">
                <td class="p-1"><input type="text" class="input-base !py-1.5 text-[10px] w-full sched-work font-bold" value="${s.workTitle || ''}" placeholder="${_t('quality.schedule.ph.work', 'Вид работ')}"></td>
                <td class="p-1"><input type="text" class="input-base !py-1.5 text-[10px] w-full sched-contr" value="${s.contractor || ''}" placeholder="${_t('quality.schedule.ph.contractor', 'Подрядчик')}"></td>
    ...
    if (activeData.length === 0) {
        rowsHtml = `<tr><td colspan="6" class="text-center py-6 text-slate-500 text-[11px] font-bold uppercase tracking-widest border-b border-dashed border-slate-300">${_t('quality.schedule.empty.no_stages', 'В графике нет этапов.')}</td></tr>`;
    }
    ...
    if (activeData.length > 0) {
        ...
        ganttHtml = `
            ...
            <details class="mb-3 group bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm overflow-hidden [&_summary::-webkit-details-marker]:hidden">
                ...
                <div class="p-3 border-t border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-[10px] leading-relaxed space-y-1.5 font-medium bg-white dark:bg-slate-800">
                    <div class="flex items-start gap-2"><span class="w-4 h-4 bg-slate-100 dark:bg-slate-700 border border-indigo-300 dark:border-indigo-600 rounded-full shrink-0 flex items-center justify-center text-[7px] font-black text-indigo-600 dark:text-indigo-400">П</span> <span><b>${_t('quality.schedule.help.ppr_label', 'ППР (-14 дн):')}</b> ${_t('quality.schedule.help.ppr_text', 'Задача на проверку и утверждение технологических карт до начала работ.')}</span></div>
                    <div class="flex items-start gap-2"><span class="w-4 h-4 bg-slate-100 dark:bg-slate-700 border border-indigo-300 dark:border-indigo-600 rounded-full shrink-0 flex items-center justify-center text-[7px] font-black text-indigo-600 dark:text-indigo-400">И</span> <span><b>${_t('quality.schedule.help.brief_label', 'Инструктаж (-7 дн):')}</b> ${_t('quality.schedule.help.brief_text', 'Сбор бригадиров, выдача TWI-инструкций и допусков.')}</span></div>
                    <div class="flex items-start gap-2"><span class="w-4 h-4 bg-slate-100 dark:bg-slate-700 border border-indigo-300 dark:border-indigo-600 rounded-full shrink-0 flex items-center justify-center text-[7px] font-black text-indigo-600 dark:text-indigo-400">Э</span> <span><b>${_t('quality.schedule.help.etalon_label', 'Эталон (-3 дн):')}</b> ${_t('quality.schedule.help.etalon_text', 'Комиссионная приемка первого образца работы.')}</span></div>
                    <div class="flex items-start gap-2"><span class="w-4 h-4 bg-slate-100 dark:bg-slate-700 border border-indigo-300 dark:border-indigo-600 rounded-full shrink-0 flex items-center justify-center text-[7px] font-black text-indigo-600 dark:text-indigo-400">С</span> <span><b>${_t('quality.schedule.help.start_label', 'Старт (0 дн):')}</b> ${_t('quality.schedule.help.start_text', 'Первая проверка выполненной работы на объекте в день старта этапа.')}</span></div>
                    <div class="flex items-start gap-2"><span class="w-4 h-4 bg-slate-100 dark:bg-slate-700 border border-indigo-300 dark:border-indigo-600 rounded-full shrink-0 flex items-center justify-center text-[7px] font-black text-indigo-600 dark:text-indigo-400">Ф</span> <span><b>${_t('quality.schedule.help.final_label', 'Финал (-3 дн от конца):')}</b> ${_t('quality.schedule.help.final_text', 'Итоговая инспекция перед закрытием объемов (КС-2), передачей фронта работ и подписания итогового акта.')}</span></div>
                </div>
            </details>
        ...
        ganttHtml = `<div class="text-center py-8 text-slate-400 text-[11px] font-bold uppercase tracking-widest bg-[var(--card-bg)] rounded-2xl border border-dashed border-[var(--card-border)] mt-4">${_t('quality.schedule.empty.gantt', 'График пуст')}</div>`;
    }
    ...
    let html = `
        ...
        <div class="px-1">
            ...
            <div class="text-[9px] text-slate-500 font-bold mt-1 mb-2">${_t('quality.schedule.viz.hint', 'Наведите мышку (или нажмите) на круглые узлы, чтобы увидеть запланированные задачи.')}</div>
            ${ganttHtml}
        </div>
    `;
    ...
    if (tbody) {
        ...
        const tr = `
            <tr class="sched-row hover:bg-[var(--hover-bg)] transition-colors" data-id="${newId}">
                <td class="p-1"><input type="text" class="input-base !py-1.5 text-[10px] w-full sched-work font-bold" value="" placeholder="${_t('quality.schedule.ph.work', 'Вид работ')}"></td>
                <td class="p-1"><input type="text" class="input-base !py-1.5 text-[10px] w-full sched-contr" value="" placeholder="${_t('quality.schedule.ph.contractor', 'Подрядчик')}"></td>

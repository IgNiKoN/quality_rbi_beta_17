...
(function () {
  ...
  function _tableBlock(tableId, headers, addLabel) {
    ...
    return '' +
      ...
      '<button onclick="window.rbi_etalonV18RemoveRow(\'' + tableId + '\')" class="px-4 bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400 py-2 rounded-lg text-[10px] font-bold uppercase active:scale-95">− ' + _t('quality.etalon.btn.remove', 'Удалить') + '</button>' +
      '</div>';
  }
  ...
  function renderMarkup() {
    ...
    html += '</button>';
    html += '<div class="text-[11px] font-black text-slate-800 dark:text-white uppercase tracking-widest text-center flex-1 truncate px-1" id="etv18-title-text">' + _t('quality.etalon.v18.title', 'Акт-Эталон (Бета)') + '</div>';
    html += '<div class="flex gap-1.5 shrink-0">';
    html += '<button onclick="saveEtalonV18Act(false)" class="text-[10px] font-bold text-slate-700 bg-slate-100 border border-slate-200 px-3 py-2 rounded-lg active:scale-95 shadow-sm transition-colors">' + _t('quality.etalon.btn.save', 'Сохранить') + '</button>';
    html += '<button onclick="saveEtalonV18Act(true)" class="text-[10px] font-bold text-white bg-indigo-600 px-3 py-2 rounded-lg active:scale-95 shadow-md transition-colors">' + _t('quality.etalon.btn.print', 'Печать') + '</button>';
    html += '</div>';
    html += '</div>';
    ...
    html += '<div class="text-[12px] font-black uppercase text-indigo-600 dark:text-indigo-400 mb-1 border-b border-slate-100 dark:border-slate-700 pb-2">' + _t('quality.etalon.section.binding', 'Привязка эталона') + '</div>';
    html += '<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">';
    html += '<div><label class="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-1 block">' + _t('quality.etalon.label.project', 'Объект *') + '</label><input type="text" id="etv18-project" autocomplete="off" class="input-base text-[12px] font-bold"></div>';
    html += '<div><label class="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-1 block">' + _t('quality.etalon.label.contractor', 'Подрядчик *') + '</label><input type="text" id="etv18-contractor" autocomplete="off" class="input-base text-[12px] font-bold"></div>';
    html += '</div>';
    html += '<div><label class="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-1 block">' + _t('quality.etalon.label.template', 'Вид работ (Чек-лист) *') + '</label><select id="etv18-template" class="input-base text-[12px] font-bold"></select></div>';
    html += '</div>';

    html += _sectionTitle('0', _t('quality.etalon.v18.section.act_data', 'Данные акта'));
    html += '<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 shadow-sm space-y-3">';
    html += '<div><label class="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-1 block">' + _t('quality.etalon.v18.label.sample_type', 'Вид эталона') + '</label>';
    ...
    html += '</div>';
    html += '<input type="text" id="etv18-type-other-text" class="input-base text-[11px] mt-1" placeholder="' + _t('quality.etalon.v18.placeholder.type_other', 'например: опытный образец фасадного узла') + '">';
    html += '</div>';
    html += '<div><label class="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-1 block">' + _t('quality.etalon.v18.label.address', 'Адрес объекта') + '</label><input type="text" id="etv18-address" class="input-base text-[11px]" placeholder="' + _t('quality.etalon.v18.placeholder.address', 'например: г. Санкт-Петербург, ул. Примерная, д. 10') + '"></div>';
    html += '<div><label class="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-1 block">' + _t('quality.etalon.v18.label.photo_object', 'Объект (для листа фотофиксации)') + '</label><input type="text" id="etv18-object" class="input-base text-[11px]"></div>';
    html += '<div><label class="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-1 block">' + _t('quality.etalon.v18.label.sample_name', 'Наименование эталона') + '</label><input type="text" id="etv18-name" class="input-base text-[11px]" placeholder="' + _t('quality.etalon.v18.placeholder.sample_name', 'например: угловое безстоечное остекление витража') + '"></div>';
    html += '<div><label class="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-1 block">' + _t('quality.etalon.v18.label.location', 'Место устройства / установки *') + '</label><input type="text" id="etv18-location" class="input-base text-[11px]" placeholder="' + _t('quality.etalon.v18.placeholder.location', 'например: корпус 1, секция 2, 1-й этаж, оси 5–6') + '"></div>';
    html += '<div><label class="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-1 block">' + _t('quality.etalon.v18.label.inspection_date', 'Дата осмотра') + '</label><input type="date" id="etv18-inspection-date" class="input-base text-[11px]"></div>';
    html += '</div>';
    ...
    html += '<th class="text-left px-2 py-1.5 font-black text-[9px] uppercase text-slate-400 border-b border-slate-200 dark:border-slate-700">' + _t('quality.etalon.v18.col.organization', 'Организация') + '</th>';
    html += '<th class="text-left px-2 py-1.5 font-black text-[9px] uppercase text-slate-400 border-b border-slate-200 dark:border-slate-700">' + _t('quality.etalon.v18.col.position', 'Должность') + '</th>';
    html += '<th class="text-left px-2 py-1.5 font-black text-[9px] uppercase text-slate-400 border-b border-slate-200 dark:border-slate-700">' + _t('quality.etalon.v18.col.name', 'Ф.И.О.') + '</th>';
    html += '</tr></thead><tbody></tbody></table></div>';
    html += '<div class="flex gap-2 mb-1">';
    html += '<button onclick="window.rbi_etalonV18AddParticipant()" class="flex-1 bg-indigo-50 text-indigo-700 dark:bg-indigo-900/20 dark:text-indigo-400 py-2 rounded-lg text-[10px] font-bold uppercase active:scale-95">+ ' + _t('quality.etalon.v18.btn.add_participant', 'Участника') + '</button>';
    html += '<button onclick="window.rbi_etalonV18RemoveParticipant()" class="px-4 bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400 py-2 rounded-lg text-[10px] font-bold uppercase active:scale-95">− ' + _t('quality.etalon.btn.remove', 'Удалить') + '</button>';
    html += '</div>';

    html += _sectionTitle('2', _t('quality.etalon.v18.section.scope', 'Состав, границы и область применения эталона'));
    html += '<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 shadow-sm space-y-3">';
    html += '<div><label class="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-1 block">' + _t('quality.etalon.v18.label.sample_composition', 'Состав и границы эталона') + '</label><textarea id="etv18-sampleComposition" class="input-base text-[11px] h-14 resize-none" placeholder="' + _t('quality.etalon.v18.placeholder.sample_composition', 'например: стойки, ригели, стеклопакет, герметизация и все согласуемые примыкания') + '"></textarea></div>';
    html += '<div><label class="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-1 block">' + _t('quality.etalon.v18.label.application_zone', 'Область применения эталона') + '</label><textarea id="etv18-applicationZone" class="input-base text-[11px] h-14 resize-none" placeholder="' + _t('quality.etalon.v18.placeholder.application_zone', 'например: все витражи первого этажа корпусов 1–3') + '"></textarea></div>';
    html += '<div><label class="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-1 block">' + _t('quality.etalon.v18.label.sample_size', 'Размер / объём образца') + '</label><input type="text" id="etv18-sampleSize" class="input-base text-[11px]" placeholder="' + _t('quality.etalon.v18.placeholder.sample_size', 'например: 1 участок размером 3,0 × 2,8 м') + '"></div>';
    html += '<div><label class="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-1 block">' + _t('quality.etalon.v18.label.not_included', 'Исключения из согласования') + '</label><textarea id="etv18-notIncluded" class="input-base text-[11px] h-14 resize-none" placeholder="' + _t('quality.etalon.v18.placeholder.not_included', 'например: внутренние откосы и чистовая отделка стен') + '"></textarea></div>';
    html += '</div>';
    ...
    html += '<th class="w-8 px-2 py-1.5 border-b border-slate-200 dark:border-slate-700">№</th>';
    html += '<th class="text-left px-2 py-1.5 font-black text-[9px] uppercase text-slate-400 border-b border-slate-200 dark:border-slate-700">' + _t('quality.etalon.v18.col.criterion', 'Критерий') + '</th>';
    html += '<th class="text-left px-2 py-1.5 font-black text-[9px] uppercase text-slate-400 border-b border-slate-200 dark:border-slate-700">' + _t('quality.etalon.v18.col.basis', 'Основание') + '</th>';
    html += '<th class="text-left px-2 py-1.5 font-black text-[9px] uppercase text-slate-400 border-b border-slate-200 dark:border-slate-700">' + _t('quality.etalon.v18.col.requirement', 'Требование') + '</th>';
    html += '<th class="text-left px-2 py-1.5 font-black text-[9px] uppercase text-slate-400 border-b border-slate-200 dark:border-slate-700">' + _t('quality.etalon.v18.col.actual', 'Факт') + '</th>';
    html += '<th class="text-left px-2 py-1.5 font-black text-[9px] uppercase text-slate-400 border-b border-slate-200 dark:border-slate-700">' + _t('quality.etalon.v18.col.compliance', 'Соотв.') + '</th>';
    html += '</tr></thead><tbody></tbody></table></div>';
    html += '<div class="flex gap-2 mb-1">';
    html += '<button onclick="window.rbi_etalonV18AddControlRow()" class="flex-1 bg-indigo-50 text-indigo-700 dark:bg-indigo-900/20 dark:text-indigo-400 py-2 rounded-lg text-[10px] font-bold uppercase active:scale-95">+ ' + _t('quality.etalon.v18.btn.add_control', 'Параметр') + '</button>';
    html += '<button onclick="window.rbi_etalonV18RemoveControlRow()" class="px-4 bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400 py-2 rounded-lg text-[10px] font-bold uppercase active:scale-95">− ' + _t('quality.etalon.btn.remove', 'Удалить') + '</button>';
    html += '</div>';
    ...
    html += '<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 shadow-sm space-y-3">';
    html += '<div class="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-1">' + _t('quality.etalon.v18.label.final_decision', 'Итоговое решение') + '</div>';
    ...
    html += '<div class="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-1 mt-3">' + _t('quality.etalon.v18.label.storage', 'Сохранность эталона') + '</div>';
    ...
    html += '<input type="text" id="etv18-storage-place" class="input-base text-[11px] mt-2" placeholder="' + _t('quality.etalon.v18.placeholder.storage_place', 'Место хранения / расположения / ответственный') + '">';
    html += '</div>';

  ...
  for (const floor of floors) {
    ...
    html += `
      <div class="flex items-center gap-2">
        <div class="w-12 shrink-0 text-center font-black text-rbi-caption text-muted bg-[var(--hover-bg)] py-3 rounded-lg border border-[var(--card-border)] uppercase tracking-tight">${_escape(
          floorLabel
        )}</div>
...
function _selectorsHtml(loc: LocSvc): string {
  ...
  return `
    <div class="flex flex-col sm:flex-row gap-2 mb-4">
      ...
      <select id="c2-transfer-building" class="flex-1 rounded-xl border border-surface bg-surface px-3 py-2.5 text-rbi-body font-bold" ${_objectId ? '' : 'disabled'}>
        ${bldOpts}
      </select>
    </div>`;
}

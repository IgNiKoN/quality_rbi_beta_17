...
function _zoomToolbarDesktopHtml(prefix: string): string {
  return `<div class="flex gap-1 shrink-0 items-center rounded-xl border border-slate-200 dark:border-slate-600 p-0.5">
    <button type="button" data-c2-zoom-out="${prefix}"
      class="w-8 h-8 rounded-lg text-[16px] font-black text-slate-600 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800" title="${_escape(_t('construction.v2.zoom_out', 'Уменьшить'))}">−</button>
    <button type="button" data-c2-zoom-in="${prefix}"
      class="w-8 h-8 rounded-lg text-[16px] font-black text-slate-600 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800" title="${_escape(_t('construction.v2.zoom_in', 'Увеличить'))}">+</button>
    <button type="button" data-c2-zoom-fit="${prefix}"
      class="px-2.5 h-8 rounded-lg text-[9px] font-black uppercase text-slate-600 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800" title="${_escape(_t('construction.v2.zoom_fit', 'По размеру'))}">Fit</button>
  </div>`;
}
